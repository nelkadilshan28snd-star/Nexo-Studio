...
import { 
  Send, History, ShieldAlert, Trash2, Paperclip, Play, Square, RotateCcw, 
  Save, Settings, Download, Upload, Cpu, Moon, Sun, FileText, Eye, CheckCircle2, XCircle 
} from 'lucide-react';
...
function App() {
  ...
  // 4. Retry Failed Iteration Logic
  const handleRetrySingle = async (item, index) => {
    const res = await executeSingleRequest(item.row);
    const updatedResults = [...bulkResults];
    updatedResults[index] = { index: item.index, row: item.row, ...res };
    setBulkResults(updatedResults);
    if (selectedResult && selectedResult.index === item.index) {
      setSelectedResult({ index: item.index, row: item.row, ...res });
    }
  };
        ...
        {bulkMode ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px', overflow: 'hidden' }}>
            <div style={{ ...glassPanel, padding: '16px', flexShrink: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {/* 3. Stop Run Button & Start Execution Button Toggle */}
                  {!bulkRunning ? (
                    <button onClick={runBulkCollection} style={{ background: '#10b981', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '11px', display: 'flex', gap: '6px' }}><Play size={13}/> Start Execution</button>
                  ) : (
                    <button onClick={handleStopBulkRun} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '11px', display: 'flex', gap: '6px' }}><Square size={13}/> Stop Execution</button>
                  )}
            ...
            <div style={{ flex: 1, display: 'flex', gap: '12px', overflow: 'hidden' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', textAlign: 'left' }}>
                  ...
                  <tbody>
                    {bulkData.map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ borderBottom: theme.panelBorder, background: selectedResult?.index === idx + 1 ? 'rgba(6,182,212,0.1)' : 'transparent' }}>
                          <td style={{ padding: '8px 12px', cursor: res ? 'pointer' : 'default' }} onClick={() => res && setSelectedResult(res)}>{idx + 1}</td>
                          <td style={{ padding: '8px 12px', fontWeight: 'bold', cursor: res ? 'pointer' : 'default' }} onClick={() => res && setSelectedResult(res)}>
                            {res ? (res.success ? <span style={{ color: '#10b981' }}>✓ PASSED</span> : <span style={{ color: '#ef4444' }}>✗ FAILED</span>) : <span style={{ color: theme.subTextColor }}>Pending</span>}
                          </td>
                          <td style={{ padding: '8px 12px', fontFamily: 'monospace', cursor: res ? 'pointer' : 'default' }} onClick={() => res && setSelectedResult(res)}>{res ? res.status : '-'}</td>
                          <td style={{ padding: '8px 12px', color: theme.subTextColor, cursor: res ? 'pointer' : 'default' }} onClick={() => res && setSelectedResult(res)}>{res ? res.time : '-'}</td>
                          <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                              {res && (
                                <button onClick={() => setSelectedResult(res)} style={{ background: 'none', border: theme.panelBorder, color: theme.textColor, padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                  <Eye size={11} /> Preview
                                </button>
                              )}
                              {res && !res.success && (
                                <button onClick={() => handleRetrySingle(res, idx)} style={{ background: '#06b6d4', border: 'none', color: '#fff', padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 'bold' }}>
              ...
              {/* 2. Postman-style Preview Request/Response Panel */}
              <div style={{ ...glassPanel, flex: '1 1 50%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ background: isDarkMode ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.03)', padding: '8px 14px', borderBottom: theme.panelBorder, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '10px', fontWeight: '700', color: theme.subTextColor, letterSpacing: '1px' }}>POSTMAN-STYLE PREVIEW INSPECTOR</span>
                  {selectedResult && (
                ...
                <div style={{ flex: 1, padding: '14px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {selectedResult ? (
                    <>
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 'bold', color: theme.subTextColor, marginBottom: '4px' }}>REQUEST DATA (Payload / Row):</div>
                        <pre style={{ margin: '0', background: isDarkMode ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0.05)', padding: '8px', borderRadius: '6px', color: '#06b6d4', fontSize: '11px', fontFamily: 'monospace', overflowX: 'auto' }}>
                          {JSON.stringify(selectedResult.row, null, 2)}
                        </pre>
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 'bold', color: theme.subTextColor, marginBottom: '4px' }}>RESPONSE DATA:</div>
                        <pre style={{ margin: '0', background: isDarkMode ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0.05)', padding: '8px', borderRadius: '6px', color: selectedResult.success ? '#10b981' : '#ef4444', fontSize: '11px', fontFamily: 'monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                          {typeof selectedResult.data === 'object' ? JSON.stringify(selectedResult.data, null, 2) : String(selectedResult.data)}
                        </pre>
                      </div>
                    </>
                  ) : (
                    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.subTextColor, fontSize: '11px' }}>Click 'Preview' on any row to inspect request and response data</div>
                  )}
                </div>
              </div>
            </div>
